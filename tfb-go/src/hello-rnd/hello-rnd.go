...
var randPool = sync.Pool{New: newRand}

func randWorldNum() int {
	r := randPool.Get().(*rand.Rand)
	v := r.Intn(worldRowCount) + 1
	randPool.Put(r)
	return v
}
...
func dbHandler(w http.ResponseWriter, r *http.Request) {
	var world World
	err := db.QueryRow(worldSelect, randWorldNum()).Scan(&world.Id, &world.RandomNumber)
	if err != nil {
...
func queriesHandler(w http.ResponseWriter, r *http.Request) {
	...
	world := make([]World, n)
	for i := 0; i < n; i++ {
		err := db.QueryRow(worldSelect, randWorldNum()).Scan(&world[i].Id, &world[i].RandomNumber)
		if err != nil {
			log.Fatalf("Error scanning world row: %s", err.Error())
		}
	}
...
func updateHandler(w http.ResponseWriter, r *http.Request) {
	...
	world := make([]World, n)
	for i := 0; i < n; i++ {
		if err := db.QueryRow(worldSelect, randWorldNum()).Scan(&world[i].Id, &world[i].RandomNumber); err != nil {
			log.Fatalf("Error scanning world row: %s", err.Error())
		}
		world[i].RandomNumber = uint16(randWorldNum())
